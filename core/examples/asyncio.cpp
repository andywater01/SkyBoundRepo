...
unsigned int __stdcall ProcessQueue(void * /*param*/)
{
    while (!gThreadQuit)
    {
        ...
        if (info)
        {
            ...
            fseek((FILE *)info->handle, info->offset, SEEK_SET);
            info->bytesread = (int)fread(info->buffer, 1, toread, (FILE *)info->handle);
...
int FMOD_Main()
{
    ...
    _beginthreadex(NULL, 0, ProcessQueue, NULL, 0, NULL);
