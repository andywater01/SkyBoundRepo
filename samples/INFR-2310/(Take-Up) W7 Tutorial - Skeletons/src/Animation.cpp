...
namespace nou
{
	...
	void SkeletalAnimClip::Update(float deltaTime, const Skeleton& skeleton)
	{
		...
		for (size_t i = 0; i < m_anim.data.size(); ++i)
		{
			...
			{
				size_t curFrame = static_cast<size_t>(m_rotFrame[i]);
				size_t nextFrame = curFrame + static_cast<size_t>(1);

				while (m_timer > anim.rotTimes[nextFrame]
					   && nextFrame < anim.rotFrames - 1)
				{
					//anim.rotKeys  = [KeyFrame0, KeyFrame1, ...]
					//anim.rotTimes = [0.0,       0.25,      ...]
					++m_rotFrame[i];
					++curFrame;
					++nextFrame;
				}

				//inverseLERP(result) = (result - start) / (end - start)
