...
int main()
{
	...
	while (!App::IsClosing() && !Input::GetKey(GLFW_KEY_ESCAPE))
	{
		...
		//TODO: How do we remove a waypoint? nm,  
		...
		if (catmullMode)
		{
			if (ImGui::Button("Current Mode: Catmull"))
			{
		...
		{
			if (ImGui::Button("Current Mode: Bezier"))
			{
		...
		//Bezier Mode
		if (ImGui::Button("Bezier Mode") && points.size() >= 6)
		{
