...
int main()
{
	...
	std::string pausePlay = "Pause";

	std::string stopLoop = "Stop Loop";
	...
	while (!App::IsClosing() && !Input::GetKey(GLFW_KEY_ESCAPE))
	{
		...
		ImGui::Begin("Animation Controller", &listPanel, ImVec2(300, 200));

		if (ImGui::Button(pausePlay.c_str()))
		{
			if (isPlaying == true)
			{
				pausePlay = "Play";
		...
		if (ImGui::Button(stopLoop.c_str()))
		{
			if (isLooping == true)
			{
				stopLoop = "Start Loop";
		...
		if (ImGui::SliderFloat("Speed", &animationSpeed, 0.0f, 4.0f))
		{
