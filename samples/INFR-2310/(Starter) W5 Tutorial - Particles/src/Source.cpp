...
std::unique_ptr<ShaderProgram> prog_texLit, prog_particles, prog_snow, prog_shuriken;
std::unique_ptr<Mesh> duckMesh;
std::unique_ptr<Texture2D> duckTex, particleTex, snowTex, shurikenTex;
std::unique_ptr<Material> duckMat, particleMat, snowMat, shurikenMat;
...
int main()
{
	...
	duckEntity.transform.m_pos = glm::vec3(-2.0f, -2.4f, 0.0f);
	duckEntity.transform.m_rotation = glm::angleAxis(glm::radians(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
	duckEntity.transform.m_rotation = glm::angleAxis(glm::radians(30.0f), glm::vec3(0.0f, 0.0f, 1.0f));

	//TODO: Set up our particle system.
	ParticleParam particleData;
	particleData.lifetime = 2.0f;
	...
	particleData.emissionRate = 60.0f;
	particleData.tanTheta = glm::tan(glm::radians(30.0f));

	ParticleParam snowData;
	snowData.lifetime = 15.0f;
	snowData.startSize = 0.5f;
	snowData.maxParticles = 200;
	snowData.emissionRate = 10.0f;
	snowData.tanTheta = glm::tan(glm::radians(30.0f));

	ParticleParam shurikenData;
    shurikenData.lifetime = 10.0f;
	shurikenData.startSize = 0.4f;
	shurikenData.startSpeed = 5.0f;
	shurikenData.maxParticles = 200;
	shurikenData.emissionRate = 15.0f;
	shurikenData.tanTheta = glm::tan(glm::radians(20.0f));

	Entity particleEntity = Entity::Create();
	particleEntity.transform.m_pos = glm::vec3(-1.8f, -2.0f, 0.0f);
	particleEntity.transform.m_rotation = glm::angleAxis(glm::radians(-50.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	particleEntity.Add<CParticleSystem>(particleEntity, *particleMat, particleData);

	Entity snowEntity = Entity::Create();
	snowEntity.transform.m_pos = glm::vec3(0.2f, 4.0f, 0.0f);
	snowEntity.transform.m_rotation = glm::angleAxis(glm::radians(180.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	snowEntity.Add<CParticleSystem>(snowEntity, *snowMat, snowData);

	Entity shurikenEntity = Entity::Create();
	shurikenEntity.transform.m_pos = glm::vec3(-1.8f, -2.0f, 0.0f);
	shurikenEntity.transform.m_rotation = glm::angleAxis(glm::radians(-50.0f), glm::vec3(0.0f, 0.0f, 1.0f));
	shurikenEntity.Add<CParticleSystem>(shurikenEntity, *shurikenMat, shurikenData);
	...
	while (!App::IsClosing() && !Input::GetKey(GLFW_KEY_ESCAPE))
	{
		...
		snowEntity.Get<CParticleSystem>().Update(deltaTime);
		shurikenEntity.Get<CParticleSystem>().Update(deltaTime);

		duckEntity.transform.RecomputeGlobal();
		particleEntity.transform.RecomputeGlobal();
		snowEntity.transform.RecomputeGlobal();
		shurikenEntity.transform.RecomputeGlobal();
		...
		snowEntity.Get<CParticleSystem>().Draw();
		shurikenEntity.Get<CParticleSystem>().Draw();
...
void LoadDefaultResources()
{
	...
	prog_snow = std::make_unique<ShaderProgram>(particles);
	prog_shuriken = std::make_unique<ShaderProgram>(particles);
	...
	snowTex = std::make_unique<Texture2D>("snowflake.png");
	shurikenTex = std::make_unique<Texture2D>("shuriken.png");
	...
	snowMat = std::make_unique<Material>(*prog_snow);
	snowMat->AddTexture("albedo", *snowTex);

	shurikenMat = std::make_unique<Material>(*prog_shuriken);
	shurikenMat->AddTexture("albedo", *shurikenTex);
}