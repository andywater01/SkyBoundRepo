...
void MeshFactory::AddCube(MeshBuilder<VertexPosNormTexCol>& mesh, const glm::mat4& transform, const glm::vec4& col) {
	...
	// Bottom
	indices[0] = mesh.AddVertex(positions[0], normals[4], uvs[0], col);
	indices[1] = mesh.AddVertex(positions[2], normals[4], uvs[1], col);
	indices[2] = mesh.AddVertex(positions[3], normals[4], uvs[2], col);
	indices[3] = mesh.AddVertex(positions[1], normals[4], uvs[3], col);
	// Top
