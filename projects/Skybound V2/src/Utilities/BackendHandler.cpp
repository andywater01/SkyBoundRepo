...
void BackendHandler::GlfwWindowResizedCallback(GLFWwindow* window, int width, int height)
{
	...
	Application::Instance().ActiveScene->Registry().view<ColorCorrectionEffect>().each([=](ColorCorrectionEffect& buf)
	{
		buf.Reshape(width, height);
	});
	Application::Instance().ActiveScene->Registry().view<WarmEffect>().each([=](WarmEffect& buf)
	{
		buf.Reshape(width, height);
	});
	Application::Instance().ActiveScene->Registry().view<CoolEffect>().each([=](CoolEffect& buf)
	{
		buf.Reshape(width, height);
	});
	...
	Application::Instance().ActiveScene->Registry().view<VignetteEffect>().each([=](VignetteEffect& buf)
		{
			buf.Reshape(width, height);
		});
}
