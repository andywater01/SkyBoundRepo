...
{
	int channels;
	stbi_set_flip_vertically_on_load(true); // Pass the filepath string as an argument

	image = stbi_load("box3.png", &width, &height, &channels, 0);
...
void loadImage2()
{
	...
	image2 = stbi_load("checker.jpg", &width2, &height2, &channels2, 0);

	if (image2)
		std::cout << ("Image Loaded: ") << width2 << " x " << height2 << " y " << std::endl;
	else
		std::cout << ("Failed to load image") << std::endl;
...
bool initGLFW() {
	...
	window = glfwCreateWindow(1000, 800, "100744494_Andrew_Waterhouse", nullptr, nullptr);
...
int main() {
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glDrawArrays(GL_TRIANGLES, 18, 36);//36
